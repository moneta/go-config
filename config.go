...
import (
	...
	logger "github.com/moneta/go-logger"
...
func New(configFiles ...string) (*viper.Viper, error) {
	...
	conf.SetEnvPrefix("CURRENT")
	conf.AutomaticEnv()

	logLevel := os.Getenv("CURRENT_LOG_LEVEL")
	if logLevel == "" {
		logLevel = "info"
	}
	...
	case 0:
		...
		configName := os.Getenv("CURRENT_CONFIG_NAME")
		configPath := os.Getenv("CURRENT_CONFIG_PATH")
